...
void Note::exportToHTML(QTextStream &stream, int indent, const HtmlExportData &exportData)
{
	QString spaces;
	...
	if (isColumn()) {
		...
		if (false/*TODO: DEBUG AND REENABLE: hasResizer()*/) {
			...
			int availableSpaceForColumnsInThisBasket = basket()->contentsWidth() - (basket()->columnsCount() - 1) * RESIZER_WIDTH;
			int availableSpaceForColumnsInBrowser    = 1024    /* typical screen width */
			                                           - 25    /* window border and scrollbar width */
			                                           - 2 * 5 /* page margin */
			                                           - (basket()->columnsCount() - 1) * RESIZER_WIDTH;
			if (availableSpaceForColumnsInThisBasket <= 0)
				availableSpaceForColumnsInThisBasket = 1;
			int widthValue = (int)(availableSpaceForColumnsInBrowser * (double) groupWidth() / availableSpaceForColumnsInThisBasket);
			if (widthValue <= 0)
				widthValue = 1;
			if (widthValue > 100)
				widthValue = 100;
		...
		FOR_EACH_CHILD (child) {
			...
			child->exportToHTML(stream, indent + 1, exportData);
			stream << '\n';
		}
	...
	QString freeStyle;
	if (isFree())
		freeStyle = " style=\"position: absolute; left: " + QString::number(x()) + "px; top: " + QString::number(y()) + "px; width: " + QString::number(groupWidth()) + "px\"";
	...
	if (isGroup()) {
		...
		FOR_EACH_CHILD (child) {
			...
			if (i == 0)
				stream << " <tr><td class=\"groupHandle\"><img src=\"" << exportData.imagesFolderName << (isFolded() ? "expand_group.png" : "fold_group.png")
				       << "\" width=\"" << EXPANDER_WIDTH << "\" height=\"" << EXPANDER_HEIGHT << "\"></td>\n";
			else if (i == 1)
				stream << " <tr><td class=\"freeSpace\" rowspan=\"" << countDirectChilds() << "\"></td>\n";
			else
				stream << " <tr>\n";
			stream << spaces.fill(' ', indent) << "  <td>";
			child->exportToHTML(stream, indent + 3, exportData);
			stream << "</td>\n"
		...
		QString additionnalClasses = content()->cssClass();
		...
		if (m_emblemsCount > 0) {
			...
			for (State::List::Iterator it = m_states.begin(); it != m_states.end(); ++it)
				if (!(*it)->emblem().isEmpty()) {
					int emblemSize = 16;
					QString iconFileName = Basket::copyIcon((*it)->emblem(), emblemSize, exportData.iconsFolderPath);
					stream << "<img src=\"" << exportData.iconsFolderName << iconFileName
		...
		stream << "<td>";
		content()->exportToHTML(stream, indent, exportData);
		stream << "</td></tr></table>";
	}
}
