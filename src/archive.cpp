...
void Archive::save(Basket *basket, bool withSubBaskets, const QString &destination)
{
	...
	for (Tag::List::iterator it = tags.begin(); it != tags.end(); ++it) {
		...
		for (State::List::iterator it2 = states.begin(); it2 != states.end(); ++it2) {
			State *state = (*it2);
			QPixmap icon =   KIconLoader::global()->loadIcon(state->emblem(), KIconLoader::Small, 16, KIconLoader::DefaultState, QStringList() /*overlays a list of emblem icons to overlay*/, /*path_store=*/0L, /*canReturnNull=*/true);
			if (!icon.isNull()) {
...
void Archive::saveBasketToArchive(Basket *basket, bool recursive, KTar *tar, QStringList &backgrounds, const QString &tempFolder, QProgressBar *progress)
{
	...
	if (!basket->icon().isEmpty() && basket->icon() != "basket") {
		QPixmap icon =  KIconLoader::global()->loadIcon(basket->icon(), KIconLoader::Small, 16, KIconLoader::DefaultState, QStringList() /*overlays a list of emblem icons to overlay*/, /*path_store=*/0L, /*canReturnNull=*/true);
		if (!icon.isNull()) {
			icon.save(tempIconFile, "PNG");
...
void Archive::importBasketIcon(QDomElement properties, const QString &extractionFolder)
{
	...
	if (!iconName.isEmpty() && iconName != "basket") {
		QPixmap icon = KIconLoader::global()->loadIcon(iconName, KIconLoader::NoGroup, 16, KIconLoader::DefaultState, QStringList(), 0L, /*canReturnNull=*/true);
		// The icon does not exists on that computer, import it:
		if (icon.isNull()) {
			QDir dir;
